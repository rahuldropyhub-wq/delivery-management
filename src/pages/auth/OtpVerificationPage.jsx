...
import { ArrowLeft, CheckCircle2, ShieldCheck, Loader2, Sparkles, AlertCircle, RefreshCw } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useToast } from '../../context/ToastContext';

export default function OtpVerificationPage() {
  const navigate = useNavigate();
  const { pendingMobile, verifyOtp, sendOtp } = useAuth();
  ...
  const handleVerify = async (e) => {
    ...
    if (result.success) {
      ...
      setErrorMessage(result.error || "Invalid OTP entered.");
    }
  };
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-xl">
          <div className="mb-6">
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1 leading-relaxed">
              OTP sent to <strong className="text-navy-900 font-semibold">{pendingMobile}</strong>
            </p>
          </div>
