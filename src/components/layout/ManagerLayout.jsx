...
export default function ManagerLayout() {
  const { isManagerAuthenticated, activeManager, setActiveManager, switchRole, logoutManager } = useAuth();
  const { data } = useData();
  ...
  return (
    <div className="min-h-screen bg-slate-100/90 text-slate-900 flex flex-col lg:flex-row">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-slate-900 text-slate-300 border-r border-slate-800 shrink-0 h-screen sticky top-0">
        ...
        {/* Shortcut to Executive View */}
        <div className="px-3 pb-2">
          <button
            onClick={handleSwitchToExecutive}
            className="w-full py-2.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700/80 text-brand-300 hover:text-white border border-brand-500/20 text-xs font-bold flex items-center justify-center gap-2 transition-colors shadow-sm"
          >
            <ExternalLink className="w-3.5 h-3.5 text-brand-400" />
            <span>Open Executive View (Rahul)</span>
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-slate-900 text-white px-4 py-3 border-b border-slate-800 flex items-center justify-between pt-safe">
          ...
          <button
            onClick={handleSwitchToExecutive}
            className="text-[11px] font-bold text-brand-300 bg-slate-800 px-2.5 py-1.5 rounded-xl border border-brand-500/30 flex items-center gap-1"
          >
            <span>User View</span>
        ...
        <header className="hidden lg:flex items-center justify-between bg-white border-b border-slate-200/80 px-8 py-3.5 sticky top-0 z-20 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleSwitchToExecutive}
              className="px-3.5 py-2 bg-blue-50 hover:bg-blue-100 text-brand-700 border border-blue-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm"
            >
              <span>Switch to Executive Portal (Rahul)</span>
